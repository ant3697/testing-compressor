...
import { X, Play, AlertTriangle, RefreshCw, Square, Zap, CheckCircle2, Info, ChevronLeft, ChevronRight, Activity, Cable, Volume2, VolumeX, Clock } from 'lucide-react';
...
export const StartingSystemModal: React.FC<StartingSystemModalProps> = ({
  ...
  // Sync authentic compressor running sound with modal simulation state
  useEffect(() => {
    if (!isOpen) {
      stopCompressorHum();
      return;
    ...
    } else if (simState === 'overload' || simState === 'idle') {
      stopCompressorHum();
    }
  }, [simState, isOpen]);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3 pb-2.5 border-b border-slate-200 dark:border-slate-800/80">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded text-tiny font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors flex items-center gap-1 cursor-pointer"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-small">
            ...
            {/* FULL WIDTH CARD: TIEMPO DE REARME DEL PTC (3 A 5 MIN) */}
            {currentVariant.includes('PTC') && (
              <div className="md:col-span-2 p-4 rounded-xl bg-sky-50 dark:bg-[#0c1322] border border-sky-200 dark:border-sky-800/60 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-sky-200/60 dark:border-sky-800/40 pb-2">
                  <h4 className="font-bold text-sky-950 dark:text-sky-200 flex items-center gap-2 text-small">
                    <Clock className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                    Tiempo de Rearme Térmico de la PTC: ¿Cuánto tarda en estar lista para un nuevo arranque?
                  </h4>
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-bold font-mono bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                    Tiempo de espera: 3 a 5 minutos
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-tiny">
                  <div className="p-3 rounded-lg bg-white dark:bg-[#0a0d16] border border-sky-100 dark:border-sky-900/50 space-y-1">
                    <strong className="text-slate-900 dark:text-white flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-blue-500"></span>
                      1. Enfriamiento por convección natural pasiva
                    </strong>
                    <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
                      En marcha, la pastilla se auto-calienta a &gt;110°C–130°C para mantenerse en alta resistencia (&gt;10.000 Ω). Al detenerse el compresor, la pastilla confinada en su cápsula plástica requiere entre 3 y 5 minutos para disipar el calor acumulado de forma natural.
                    </p>
                  </div>
                  ...
                  <div className="p-3 rounded-lg bg-white dark:bg-[#0a0d16] border border-sky-100 dark:border-sky-900/50 space-y-1">
                    <strong className="text-slate-900 dark:text-white flex items-center gap-1.5">
                      ...
                      2. Descenso bajo la temperatura de Curie (~70°C)
                    </strong>
                    <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
                      Para volver a conducir la corriente de arranque (10 a 15 A), su temperatura debe bajar de ~70°C. Solo entonces su resistencia cae bruscamente a 15–25 Ω, reactivando la bobina auxiliar de arranque.
                    </p>
                  </div>

                  <div className="p-3 rounded-lg bg-rose-50/60 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 space-y-1">
                    <strong className="text-rose-900 dark:text-rose-300 flex items-center gap-1.5">
                      <AlertTriangle className="w-3.5 h-3.5 text-rose-500" />
                      3. Comportamiento ante microcortes (5 a 15 s)
                    </strong>
                    <p className="text-rose-800 dark:text-rose-200/90 leading-relaxed">
                      Si la luz se corta y regresa enseguida, la PTC caliente bloquea la corriente de arranque. El compresor no arranca, absorbe la corriente de rotor bloqueado (LRA) y a los 3-8 segundos salta el protector Klixon por sobreintensidad.
                    </p>
                  </div>

                  <div className="p-3 rounded-lg bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 space-y-1">
                    <strong className="text-amber-900 dark:text-amber-300 flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5 text-amber-600" />
                      4. Retardo anti-reciclo en termostatos
                    </strong>
                    <p className="text-amber-800 dark:text-amber-200/90 leading-relaxed">
                      Los termostatos digitales (Carel, Dixell, Eliwell, etc.) se programan con un retardo anti-reciclo de 3 a 5 min, asegurando el enfriamiento de la PTC y la igualación de presiones capilares antes del re-arranque.
                    </p>
                  </div>
                </div>
              </div>
